...
async def generate_care_instructions(request: CareInstructionCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        chat = LlmChat(
            ...
            session_id=f"instruction_{request.appointment_id}",
            system_message="""Você é um assistente médico especializado em criar orientações pós-atendimento 
            claras e personalizadas para pacientes. Use linguagem simples, acessível e empática. 
            Inclua: cuidados gerais, medicações (se mencionadas), sinais de alerta, e quando retornar.
            Responda sempre em português brasileiro."""
        )
        chat.with_model("openai", "gpt-5.2")
        
        prompt = f"""Gere orientações de pós-atendimento personalizadas para:
        ...
        Crie orientações detalhadas incluindo:
        1. Cuidados gerais nas próximas 24-48 horas
        2. Medicações e como tomar (se aplicável)
        3. Sinais de alerta para procurar atendimento
        4. Restrições de atividades
        5. Próximos passos e retorno"""
